...
export default function Hello() {
  ...
  const getRandomImageIndex = () => Math.floor(Math.random() * imageCount);

  function getNextImageIndex(currentIndex: number) {
    return (currentIndex + 1) % 4;
  }

  function getImageURL(index: number) {
    return `/images/hello/${index}.jpg`;
  }

  function preLoadImage(index: number) {
    const image = new Image();
    image.src = getImageURL(getNextImageIndex(index));
  }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setImageIndex(getNextImageIndex(imageIndexRef.current));
    }, 4000);
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center">
        <div className="flex flex-col items-center mb-2">
          <img src={getImageURL(imageIndex)} className="rounded-full h-44" />
        </div>
